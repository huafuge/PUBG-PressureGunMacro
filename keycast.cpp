...
//xiesi
CTimer yaqiangTimer;
vector<gun> m_vctGuns;
vector<gun>::iterator iter;
cfg m_cfg[7];
int m_nIndexCfg;
string str = "fdsf";
...
void updateLabel(int i)
{
	...
	if (keyLabels[i].length > 0) {
		...
		int bgAlpha = (int)(r*labelSettings.bgOpacity), textAlpha = 100, borderAlpha = 0;
		bgAlpha = 0;
 ...
 */
void showText2(LPCWSTR text, int behavior = 0) {
...
void showText(LPCWSTR text, int behavior = 0) {
	if (m_bIsStart == false)
	{
		...
		char* tmp;
		string ss = "1";
		//sprintf(tmp, L"[%s]-[%d]-[%d]-[%s]", m_bIsAuxiliary == true ? L"on" : L"off", iter->jiange, iter->lidu, iter->name.c_str());
		CString str = "";

		str.Format(L"[%d]-[%s]-[%s]-[%d]-[%d]-[%s]", m_nIndexCfg + 1, m_cfg[m_nIndexCfg].m_bSwitch == true ? L"on" : L"off", iter->bauto ? L"auto" : L"single", iter->jiange, iter->lidu, iter->name);
...
void prepareLabels() {
	...
	for (DWORD i = 0; i < labelCount; i++) {
		...
		keyLabels[i].rect.Y = 0 + unitH*(i + offset) + labelSpacing + labelSettings.borderSize;
...
void loadSettings() {
	...
	GetPrivateProfileString(L"KeyCastOW", L"branding", L"", branding, BRANDINGMAX, iniFile);
...
void SaveAllGun()
{
	...
	//在对象上添加键值对
	cJSON *arraycfg = NULL;
	cJSON_AddItemToObject(json, "cfg", arraycfg = cJSON_CreateArray());
	for (size_t i = 0; i < 7; i++)
	{
		//在数组上添加对象
		cJSON *obj = NULL;
		cJSON_AddItemToArray(arraycfg, obj = cJSON_CreateObject());

		cJSON_AddNumberToObject(obj, "index", m_cfg[i].m_nIndex);
		cJSON_AddNumberToObject(obj, "switch", m_cfg[i].m_bSwitch ? 1 : 0);
	}

	//在对象上添加键值对
	cJSON *array = NULL;
	cJSON_AddItemToObject(json, "guns", array = cJSON_CreateArray());
	for each (gun var in m_vctGuns)
	{
		//在数组上添加对象
		cJSON *obj = NULL;
		cJSON_AddItemToArray(array, obj = cJSON_CreateObject());

		LPTSTR lpsz = new TCHAR[var.name.GetLength() + 1];
		_tcscpy(lpsz, var.name);
		char chartmp[128];
		wcharTochar(lpsz, chartmp, sizeof(chartmp));

		cJSON_AddItemToObject(obj, "name", cJSON_CreateString(chartmp));
		cJSON_AddNumberToObject(obj, "jiange", var.jiange);
		cJSON_AddNumberToObject(obj, "lidu", var.lidu);
		cJSON_AddNumberToObject(obj, "auto", var.bauto);
	}

	//清理工作
	FILE *fp = fopen("guns.json", "w");
...
void LoadAllGun()
{
	...
	f = fopen("guns.json", "rb");fseek(f, 0, SEEK_END);len = ftell(f);fseek(f, 0, SEEK_SET);
	...
	else
	{
		...
		node = cJSON_GetObjectItem(json, "cfg");
		if (node->type == cJSON_Array)
		{
			...
			for (int i = 0;i < size;i++)
			{
				...
				m_cfg[i].m_nIndex = cJSON_GetObjectItem(tnode, "index")->valueint;
				m_cfg[i].m_bSwitch = cJSON_GetObjectItem(tnode, "switch")->valueint == 1 ? true : false;
			}
		}

		node = cJSON_GetObjectItem(json, "guns");
		if (node->type == cJSON_Array)
		{
			printf("array size is %d", cJSON_GetArraySize(node));
			cJSON *tnode = NULL;
			...
			for (int i = 0;i < size;i++)
			{
				tnode = cJSON_GetArrayItem(node, i);
				gun tmpgun;
				tmpgun.name = cJSON_GetObjectItem(tnode, "name")->valuestring;
				tmpgun.jiange = cJSON_GetObjectItem(tnode, "jiange")->valueint;
				tmpgun.lidu = cJSON_GetObjectItem(tnode, "lidu")->valueint;
				tmpgun.bauto = cJSON_GetObjectItem(tnode, "auto")->valueint == 1 ? true : false;
				m_vctGuns.push_back(tmpgun);
			}
		}
	}
...
void OpenOrClose()
{
	//m_cfg[m_nIndexCfg].m_bSwitch = !m_cfg[m_nIndexCfg].m_bSwitch;
	//SaveAllGun();
}
...
void YaQiang(bool _bBegin)
{
	if (_bBegin)
	{
		yaqiangTimer.Start(iter->jiange);
	}
	else
	{
		yaqiangTimer.Stop();
...
void OnTimeYaQiang()
{
	if (m_cfg[m_nIndexCfg].m_bSwitch)
	{
		mouse_event(MOUSEEVENTF_MOVE, 0, iter->lidu, 0, 0);
		if (!iter->bauto)
		{
			yaqiangTimer.Stop();
		}
...
void ChangeGun(bool _bIsNext)
{
	if (_bIsNext)
	{
		if (iter + 1 == m_vctGuns.end())
		{
			iter = m_vctGuns.begin();
		}
		else
		{
			iter++;
	...
	{
		if (iter == m_vctGuns.begin())
		{
			iter = m_vctGuns.end() - 1;
		}
		else
		{
			iter--;
	...
	m_cfg[m_nIndexCfg].m_nIndex = iter - m_vctGuns.begin();
	SaveAllGun();
}
...
{
	m_nIndexCfg = _nIndex;
	iter = m_vctGuns.begin() + m_cfg[m_nIndexCfg].m_nIndex;
}
	...
	LPSTR lpszArgs, int nWinMode)
{
	...
	lstrcpyW(iniFile, L".\\setting.exe");
	...
	SetWindowPos(hDlgSettings, 0, 0, 0, 200, 150, 2);
	...
	yaqiangTimer.OnTimedEvent = OnTimeYaQiang;
